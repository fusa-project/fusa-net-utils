...
class HTSAT_Swin_Transformer(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, 
        ...
        self.patch_embed = PatchEmbed(
            ...
            embed_dim=self.embed_dim, norm_layer=self.norm_layer, patch_stride = self.patch_stride)
        ...
        if self.enable_tscam:
            ...
            self.tscam_conv = nn.Conv2d(
                ...
                out_channels = self.classes_num,
                kernel_size = (SF,3),
                padding = (0,1)
            )
            self.head = nn.Linear(classes_num, classes_num)
        else:
            self.head = nn.Linear(self.num_features, classes_num) if classes_num > 0 else nn.Identity()
    ...
    def forward(self, input: torch.Tensor, mixup_lambda = None, infer_mode = False):# out_feat_keys: List[str] = None):
        """
        ...
        if infer_mode:
            ...
            if x.shape[2] > self.freq_ratio * self.spec_size:
                ...
                output_dict = self.forward_features(x)
        # x = self.head(x)
        return output_dict["framewise_output"]
